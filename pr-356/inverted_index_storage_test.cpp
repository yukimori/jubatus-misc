...
TEST(inverted_index_storage, trivial) {
  ...
  for(int i=0; i<scores.size(); ++i) {
    std::cout << std::setprecision(32) << scores[i].first << ":" << scores[i].second << ":" << std::min(scores[i].second, 1.f) << std::endl;
  }

  ASSERT_EQ(4u, scores.size());

  EXPECT_EQ(1, scores[0].second);
  EXPECT_EQ("r4", scores[0].first);
  EXPECT_FLOAT_EQ(2.0 / std::sqrt(3) / std::sqrt(2), scores[1].second);
  EXPECT_EQ("r1", scores[1].first);
  EXPECT_FLOAT_EQ(1.0 / std::sqrt(2) / std::sqrt(2), scores[2].second);
  EXPECT_EQ("r3", scores[2].first);
  EXPECT_FLOAT_EQ(1.0 / std::sqrt(2) / std::sqrt(3), scores[3].second);
  EXPECT_EQ("r2", scores[3].first);
  ...
  ASSERT_EQ(4u, scores2.size());
  EXPECT_FLOAT_EQ(1.0, scores2[0].second);
  EXPECT_EQ("r4", scores2[0].first);
  EXPECT_FLOAT_EQ(2.0 / std::sqrt(3) / std::sqrt(2), scores2[1].second);
  EXPECT_EQ("r1", scores2[1].first);
  EXPECT_FLOAT_EQ(1.0 / std::sqrt(2) / std::sqrt(2), scores2[2].second);
  EXPECT_EQ("r3", scores2[2].first);
  EXPECT_FLOAT_EQ(1.0 / std::sqrt(2) / std::sqrt(3), scores2[3].second);
  EXPECT_EQ("r2", scores2[3].first);
}
